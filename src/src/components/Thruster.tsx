...
const getMarks = (from: number, to: number, step: number) => {
  const values = [...Array(Math.floor((to - from) / step) + 1)].map(
    (_, i) => from + i * step
  );
...
export const Thruster = ({
  ...
  //   const thrusterEffortPublisher = useROSTopicPublisher<any>(
  ...
  function ThrusterControlThumbComponent(props: any) {
    return (
      <span {...props}>
        {!thumbEnabled ? (
          <img src={RedButtonImg} width="100%" height="100%" />
        ) : null}
      </span>
    );
  }
